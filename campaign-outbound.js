...
fastify.register(async (fastifyInstance) => {
  fastifyInstance.get(
    ...
    (connection, req) => {
      ...
      // MONITORING: Track call metrics
      const callStartTime = Date.now();
      let audioChunksReceived = 0;
      let audioChunksSent = 0;
      let endCallAttempts = [];
      let closingPhraseChecks = [];
      
      const logMetric = (type, data) => {
        const timestamp = new Date().toISOString();
        const elapsed = Math.floor((Date.now() - callStartTime) / 1000);
        console.log(`[METRIC][${elapsed}s] ${type}:`, JSON.stringify(data));
      };
      ...
      const safeEndCall = (reason, details = {}) => {
        // MONITORING: Log every end call attempt
        const attempt = {
          timestamp: Date.now(),
          reason,
          closingPhraseDetected,
        ...
        if (!closingPhraseDetected) {
          ...
          endCallAttempts.push(attempt);
          logMetric('END_CALL_BLOCKED', attempt);
        ...
        endCallAttempts.push(attempt);
        logMetric('END_CALL_ALLOWED', attempt);
      ...
      const checkForClosingPhrase = (text) => {
        ...
        // MONITORING: Log every closing phrase check
        const checkData = {
        ...
        logMetric('CLOSING_PHRASE_CHECK', checkData);
        closingPhraseChecks.push({ timestamp: Date.now(), ...checkData });
        ...
        for (const phrase of closingPhrases) {
          ...
          if (phraseIndex !== -1) {
            ...
            // MONITORING: Log phrase match
            logMetric('CLOSING_PHRASE_FOUND', {
              phrase,
              phraseIndex,
              phraseAtEnd,
          ...
          switch (msg.event) {
                ...
                elevenLabsWs.on("message", (data) => {
                  try {
                    ...
                    if (message.type === "audio" && streamSid) {
                      ...
                      // MONITORING: Track audio chunks
                      audioChunksSent++;
                      if (audioChunksSent % 100 === 0) {
                        const elapsed = Math.floor((Date.now() - callStartTime) / 1000);
                        logMetric('AUDIO_PROGRESS', {
                          elapsed,
            ...
            case "media":
              if (elevenLabsWs?.readyState === WebSocket.OPEN && elevenLabsReady) {
                // MONITORING: Track incoming audio from user
                audioChunksReceived++;
      ...
      ws.on("close", () => {
        ...
        // MONITORING: Generate call summary
        const callSummary = {
          call_sid: callSid,
          ...
          closing_phrase_detected: closingPhraseDetected,
          closing_phrase_checks: closingPhraseChecks.length,
          end_call_attempts: endCallAttempts.length,
          blocked_attempts: endCallAttempts.filter(a => a.blocked).length,
          allowed_attempts: endCallAttempts.filter(a => !a.blocked).length,
          last_agent_response: lastAgentResponse.substring(0, 200),
          final_reason: endCallAttempts[endCallAttempts.length - 1]?.reason || 'unknown'
        };
        
        console.log('\n' + '='.repeat(80));
        console.log('[CALL SUMMARY]');
        console.log(JSON.stringify(callSummary, null, 2));
        console.log('\n[END CALL ATTEMPTS DETAIL]');
        endCallAttempts.forEach((attempt, idx) => {
          console.log(`  ${idx + 1}. [${attempt.blocked ? 'BLOCKED' : 'ALLOWED'}] ${attempt.reason} (${attempt.callDuration}s)`);
        });
        console.log('='.repeat(80) + '\n');
        
        // Send to logging endpoint
        if (callSid) {
          fetch('https://innovationstudio.be/Scraper/log-call-termination.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(callSummary)
          }).catch(err => console.error('[MONITORING] Failed to log to endpoint:', err));
        }
